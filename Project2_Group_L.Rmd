...
We can continue to evaluate calories, fat, carbohydrates to see if there are interesting finds. Let's compare fat to carbohydrates, in order to choose food based on fat and carbohydrate levels. First, we'll add a new output variable `calories_level`. This variable did not exist in the source data and had to be created using the values of calories. The categories utilized are: 
